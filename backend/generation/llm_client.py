    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            + (
                ", and that Ollama is running for OFFLINE_MODE."
                if OFFLINE_MODE
                else "."
            )
        ) from exc
    ...
    while True:
        try:
            chunk = await asyncio.wait_for(
                stream_iter.__anext__(), timeout=STREAM_CHUNK_TIMEOUT
            )
        except StopAsyncIteration:
            return
...
async def agenerate(
    ...
    prompt = build_user_prompt(
        query, chunks, formulation_category, statutory_tags, formulation_notes
    )
...
if __name__ == "__main__":
    ...
    query = (
        " ".join(sys.argv[1:])
        or "What does Section 3(p) say about traditional knowledge?"
    )
