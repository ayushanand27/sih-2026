...
class GraphState(TypedDict, total=False):
    ...
    jurisdiction: (
        str  # "india" or "international" — see ingestion.indexer.JURISDICTIONS
    )
    formulation_category: str  # see graph.formulation.FORMULATION_CATEGORIES
    formulation_notes: list[
        str
    ]  # deterministic, code-authored legal-context strings — see graph.formulation.triage_formulation
    ...
    clarifying_questions: list[
        str
    ]  # informational — generation proceeds regardless, using the first-matched category
    rewritten_query: str
    candidates: list[dict]  # fused top-20, before reranking
    bm25_top_score: (
        float  # top raw BM25 score from this round's retrieve() — diagnostic only
    )
    reranked: list[
        dict
    ]  # reranked top-5, rerank_score is a calibrated 0-1 confidence (see retrieval/reranker.py)
    ...
    related_provisions: list[
        dict
    ]  # knowledge-graph cross-references — see graph_kg/kg.py
    flags: dict
