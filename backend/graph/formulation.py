...
_CATEGORY_PATTERNS: list[tuple[str, re.Pattern]] = [
    (
        "ayurveda_aahar",
        re.compile(
            r"\b(nutraceutical|ayurveda[\s-]*aahar|dietary supplement|functional food|food product)\b",
            re.I,
        ),
    ),
    (
    ...
    (
        "phytopharmaceutical",
        re.compile(
            r"\b(phytopharmaceutical|botanical drug|standardi[sz]ed (botanical )?extract)\b",
            re.I,
        ),
    ),
    (
...
def triage_formulation(query: str) -> dict:
    ...
    """
    matched = [
        category for category, pattern in _CATEGORY_PATTERNS if pattern.search(query)
    ]
    ...
    notes = (
        [CUSTOM_COMBINATION_NOTE]
        if (is_custom_combination and "patent_and_proprietary" in matched)
        else []
    )
