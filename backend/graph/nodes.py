...
async def retrieve(state: GraphState) -> dict:
    ...
    bm25_results = await asyncio.to_thread(
        bm25_search_sync, query, top_k=fused_k, jurisdiction=jurisdiction
    )
...
async def rerank_node(state: GraphState) -> dict:
    ...
    flags["weak_grounding"] = (
        bm25_top_score >= BM25_STRONG_MATCH
        and top_confidence < WEAK_GROUNDING_CONFIDENCE
    )
