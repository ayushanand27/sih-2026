...
async def lifespan(_app: FastAPI):
    ...
    try:
        ...
        await conn.close()
        log.info(
            "Startup warm-up complete: graph, embedding model, reranker, DB connection."
        )
    except Exception:
        log.exception(
...
class Citation(BaseModel):
    ...
    source_file: str = Field(
        description="The source PDF's filename — display this as the citation."
    )
...
class FormCard(BaseModel):
    ...
    form_id: str
    agency: str = Field(
        description='"NBA" (National Biodiversity Authority) or "IPO" (Indian Patent Office).'
    )
    jurisdiction: str = Field(
        description='Always "india" — see compliance/form_navigator.py.'
    )
    title: str
...
@app.get(
    ...
    responses={
        400: {
            "description": "filename contains a path separator or '..' — rejected before any file lookup."
        },
        404: {
            "description": "No file by that exact name in data/ or data/international/."
        },
    },
)
def get_source(filename: str):
    ...
    raise HTTPException(
        status_code=404, detail=f"No source file named {safe_name!r} found."
    )
...
async def run_query(
    ...
    chunk_tags = sorted(
        {
            tag
            for chunk in (result.get("reranked") or [])
    ...
    compliance_flags = (
        [] if flags.get("abstained") else flag_compliance_checkpoints(chunk_tags)
    )
...
@app.post(
    ...
    responses={
        422: {
            "description": "Invalid request body (e.g. jurisdiction not 'india' or 'international')."
        },
        503: {
            "description": "Groq (or Ollama, under OFFLINE_MODE) could not be reached."
        },
...
async def query_stream(req: QueryRequest):
    ...
    async def event_generator():
        try:
            ...
            compliance_flags = (
                [] if abstained else flag_compliance_checkpoints(chunk_tags)
            )
            ...
            with_notes = append_formulation_notes(
                raw_answer, None if abstained else formulation_notes
            )
...
@app.post(
    ...
    responses={
        401: {
            "description": "Missing or invalid X-Admin-Token (only when ADMIN_TOKEN is set)."
        },
...
async def voice_query(
    ...
    response_language = (
        detected_language if detected_language in TARGET_LANGUAGE_CODES else "en-IN"
    )
